...
class GraphDrawer(QWidget):
    ...
    def initUI(self):
        ...
        self.train_image=QImage(os.path.join(directory, "train.png"))        
        self.show()
    ...
    def paintEvent(self, e):
        if self.graph:
            ...
            train_line_position=[]
            for train in self.player_info['trains']:
                train_line_position.append([train['line_idx'],train['position']] )
            full_trains_info=[]
            for edge in self.graph.edges:
                p1, p2 = edge.points
                x1, y1, x2, y2 = (
                ...
                train_line_position_count = len(train_line_position)
                i=0
                while i < train_line_position_count:
                    train_line_idx = train_line_position[i][0]
                    train_position = train_line_position[i][1]
                    if edge.idx == train_line_idx:
                        train_x=x1 + (x2 - x1) * (train_position/10)-self.train_image.size().width()
                        train_y=y1 + (y2 - y1) * (train_position/10)-self.train_image.size().height()/2
                        full_trains_info.append([train_line_idx,train_x,train_y])
                        train_line_position.remove(train_line_position[i])
                        train_line_position_count-=1
                        continue
                    i+=1
            ...
            for _,train_x,train_y in full_trains_info:
                painter.drawImage(QPoint(train_x,train_y),self.train_image)
            painter.end()
